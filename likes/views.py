...
from .models import LikeCount, LikeRecord, DislikeCount, DisikeRecord
...
def dislike_change(request):
    ...
    if is_like == 'true':
        # 要点赞
        dislike_record, created = DisikeRecord.objects.get_or_create(content_type=content_type, object_id=object_id, user=user)
        if created:
            # 未点赞，进行点赞
        ...
        # 取消点赞
        if DisikeRecord.objects.filter(content_type=content_type, object_id=object_id, user=user).exists():
            dislike_record = DisikeRecord.objects.get(content_type=content_type, object_id=object_id, user=user)
